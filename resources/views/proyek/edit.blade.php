...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-gradient-to-r from-primary-600 to-accent-500 rounded-2xl shadow-lg p-6 mb-6 text-white">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-2xl font-bold">Edit Proyek</h2>
                <p class="text-primary-100 text-sm mt-1">Perbarui informasi proyek</p>
            </div>
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('proyek.update', $proyek) }}" class="space-y-6">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" name="kode_proyek" value="{{ old('kode_proyek', $proyek->kode_proyek) }}" required
                ...
                <div class="relative">
                    ...
                    <select name="kategori" required class="w-full pl-12 pr-4 py-3.5 rounded-xl border-2 border-gray-200 focus:border-primary-500 focus:ring-4 focus:ring-primary-100 transition-all text-sm appearance-none bg-white">
                        ...
                        @foreach(\App\Models\Proyek::$kategoriRoles as $kategori => $roles)
                            <option value="{{ $kategori }}" {{ old('kategori', $proyek->kategori) == $kategori ? 'selected' : '' }}>{{ $kategori }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <select name="klien_id" required class="w-full pl-12 pr-4 py-3.5 rounded-xl border-2 border-gray-200 focus:border-primary-500 focus:ring-4 focus:ring-primary-100 transition-all text-sm appearance-none bg-white">
                        ...
                        @foreach($kliens as $k)
                            <option value="{{ $k->id }}" {{ old('klien_id', $proyek->klien_id) == $k->id ? 'selected' : '' }}>{{ $k->nama_klien }} ({{ $k->nama_perusahaan }})</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" name="lokasi_proyek" value="{{ old('lokasi_proyek', $proyek->lokasi_proyek) }}"
                        class="w-full pl-12 pr-4 py-3.5 rounded-xl border-2 border-gray-200 focus:border-primary-500 focus:ring-4 focus:ring-primary-100 transition-all text-sm"
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="4"
                    ...
                    placeholder="Jelaskan detail proyek...">{{ old('deskripsi', $proyek->deskripsi) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="date" name="tanggal_mulai" value="{{ old('tanggal_mulai', optional($proyek->tanggal_mulai)->format('Y-m-d')) }}"
                            class="w-full pl-12 pr-4 py-3.5 rounded-xl border-2 border-gray-200 focus:border-primary-500 focus:ring-4 focus:ring-primary-100 transition-all text-sm">
                    </div>
                </div>
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="date" name="tanggal_selesai" value="{{ old('tanggal_selesai', optional($proyek->tanggal_selesai)->format('Y-m-d')) }}"
                            class="w-full pl-12 pr-4 py-3.5 rounded-xl border-2 border-gray-200 focus:border-primary-500 focus:ring-4 focus:ring-primary-100 transition-all text-sm">
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-3 gap-3">
                    <label class="relative flex items-center p-4 border-2 border-gray-200 rounded-xl cursor-pointer hover:border-primary-500 transition-all {{ old('status_proyek', $proyek->status_proyek) == 'berjalan' ? 'border-primary-500 bg-primary-50' : '' }}">
                        <input type="radio" name="status_proyek" value="berjalan" {{ old('status_proyek', $proyek->status_proyek) == 'berjalan' ? 'checked' : '' }} class="mr-3">
                        <div>
                            <p class="font-semibold text-sm text-gray-800">Berjalan</p>
                            <p class="text-xs text-gray-500">Sedang dikerjakan</p>
                        </div>
                    </label>
                    <label class="relative flex items-center p-4 border-2 border-gray-200 rounded-xl cursor-pointer hover:border-amber-500 transition-all {{ old('status_proyek', $proyek->status_proyek) == 'tertunda' ? 'border-amber-500 bg-amber-50' : '' }}">
                        <input type="radio" name="status_proyek" value="tertunda" {{ old('status_proyek', $proyek->status_proyek) == 'tertunda' ? 'checked' : '' }} class="mr-3">
                        <div>
                            <p class="font-semibold text-sm text-gray-800">Tertunda</p>
                            <p class="text-xs text-gray-500">Ditunda sementara</p>
                        </div>
                    </label>
                    <label class="relative flex items-center p-4 border-2 border-gray-200 rounded-xl cursor-pointer hover:border-green-500 transition-all {{ old('status_proyek', $proyek->status_proyek) == 'selesai' ? 'border-green-500 bg-green-50' : '' }}">
                        <input type="radio" name="status_proyek" value="selesai" {{ old('status_proyek', $proyek->status_proyek) == 'selesai' ? 'checked' : '' }} class="mr-3">
                        <div>
